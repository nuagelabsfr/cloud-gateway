...
{
    fprintf(stderr, "Usage: CloudGatewayRemoveFilesystem [OPTIONS]\n");
...
#include "cg_config_remove_fs.itm"
#undef ITEM
            ...
            file != NULL)
        {
            ...
            if (result == 0)
            {
                ...
                if (result == 0)
                {
                    ...
                    if (result == 0)
                    {
                        result = cgutils_xml_writer_element_remove_from_tree(elt);
                        ...
                        else
                        {
                            ...
                            fprintf(stderr, "Error removing filesystem %s: %s\n",
                                    fs_id,
                                    strerror(result));
                        }
                    }
                    ...
                    {
                        fprintf(stderr, "Error looking for a filesystem named %s: %s\n",
                                fs_id,
                                strerror(result));
                    }
