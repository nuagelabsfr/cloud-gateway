...
{
    fprintf(stderr, "Usage: CloudGatewayListFilesystems [OPTIONS]\n");
        ...
        if (file != NULL)
        {
            ...
            if (result == 0)
            {
                ...
                if (result == 0)
                {
                    ...
                    for (elt = cgutils_llist_get_first(fs_list);
                         elt != NULL;
                         elt = cgutils_llist_elt_get_next(elt))
                    {
                        cgutils_xml_reader * fs = cgutils_llist_elt_get_object(elt);
                        char * id = NULL;

                        result = cgutils_xml_reader_get_string(fs,
                                                               "Id",
                                                               &id);
                        ...
                        {
                            fprintf(stderr, "Error getting filesystem's id: %s\n",
                                    strerror(result));
                        }
                    }

                    cgutils_llist_free(&fs_list, &cgutils_xml_reader_delete);
                }
                ...
                {
                    fprintf(stderr, "Error getting filesystems list: %s\n",
                            strerror(result));
                }
