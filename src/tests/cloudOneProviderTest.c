...
static char const * test_small_file_path = NULL;
static char const * test_huge_file_path = NULL;
    ...
    if (result == 0)
    {
        ...
        if (fd != NULL)
        {
            *fd = -1;
            ...
            result = cgutils_file_open(test_small_file_path,
                                       O_RDONLY | O_NONBLOCK,
                                       0,
                                       fd);
    ...
    if (result == 0)
    {
        ...
        if (fd != NULL)
        {
            *fd = -1;
            ...
            result = cgutils_file_open(test_huge_file_path,
                                       O_RDONLY | O_NONBLOCK,
                                       0,
                                       fd);
         ...
         char const ** const argv)
{
    ...
    if (argc == 4)
    {
        ...
        test_small_file_path = argv[2];
        test_huge_file_path = argv[3];

        result = cg_tests_init_all();
        ...
        if (result == 0)
        {
           ...
           cgutils_set_color(stderr, CLOUDUTILS_ANSI_COLOR_ATTR_RESET, CLOUDUTILS_ANSI_COLOR_WHITE, CLOUDUTILS_ANSI_COLOR_BLACK);
            ...
            result = test_provider_init(file);

            if (result == 0)
            {
                test_provider_test_suite("Instance1");
    ...
    {
        CGUTILS_ERROR("Usage: %s <config file> <small file path> <huge file path>\n",
                      argv[0]);
    }
